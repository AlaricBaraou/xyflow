...
export function adoptUserNodes<NodeType extends NodeBase>(
  ...
  // in-place fast path: reconcile only the changed entries in nodeLookup instead of clone + clear +
  // rebuild. The structure is unchanged (canIncrementalAdopt guarantees no add / remove / reorder /
  // reparent), so parentLookup stays valid as is; updateChildNode just refreshes the entries it
  // touches. A node's absolute position / z is recomputed only when it changed itself or its parent
  // moved, so a drag is O(changed + cascaded descendants), not O(parented).
