...
function SelectionListenerInner<NodeType extends Node = Node, EdgeType extends Edge = Edge>({
  ...
  /*
   * Read the selected nodes/edges through the selection channel: it notifies only when the selected
   * set changes (setNodes/setEdges diff against the store's own selected ids), so the O(N+E) scan
   * no longer runs on every store emit (each drag frame). The areEqual dedup keeps a stable
   * reference so the effect fires only on a real selection change.
   */
