 ...
 * culling, on a viewport/endpoint-position change. We subscribe to the edge-list channel (and, only
 * when culling, to the store for viewport/positions) and recompute the snapshot only then, instead
 * of a global `useStore` selector that rebuilt the id array on every store emit.
 *
 * @internal
